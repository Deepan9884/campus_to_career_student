 ...
 * so questions, options, and explanations display clean, professional text
...
export function formatMathText(text: string | null | undefined): string {
  ...
  const cleanMathSegment = (math: string): string => {
    let s = math.trim();
    ...
    s = s.replace(/\^\{([0-9nki+-=()xy]+)\}/g, (_, exp) =>
      exp.split("").map((ch: string) => superscripts[ch] || ch).join("")
    );
