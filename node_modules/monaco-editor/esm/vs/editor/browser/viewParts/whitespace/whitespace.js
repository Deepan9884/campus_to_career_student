...
class WhitespaceOverlay extends DynamicViewOverlay {
    ...
    _applyRenderWhitespace(ctx, lineNumber, selections, lineData) {
        ...
        for (let charIndex = fauxIndentLength; charIndex < len; charIndex++) {
            ...
            if (USE_SVG) {
                ...
                if (chCode === 9 /* CharCode.Tab */) {
                    ...
                    // CampusToCareer: enlarged dots aligned with lowercase letter centers
                    const dotR = Math.max(2.4, Math.min(3.6, spaceWidth * 0.38));
                    result += `<circle cx="${(visibleRange.left + spaceWidth / 2).toFixed(2)}" cy="${(lineHeight * 0.58).toFixed(2)}" r="${dotR.toFixed(2)}" />`;
                }
            }
            else {
                if (chCode === 9 /* CharCode.Tab */) {
        ...
        if (USE_SVG) {
            maxLeft = Math.round(maxLeft + spaceWidth * 4);
    ...
    _renderArrow(lineHeight, spaceWidth, left) {
        // CampusToCareer: Full-span CodeTantra-style tab arrow (──────►) spanning full 4-column tab
        const tabWidth = spaceWidth * 4;
        const sw = Math.max(1.6, spaceWidth * 0.18);
        const dy = lineHeight * 0.58;
        const startX = left + sw;
        const endX = left + tabWidth - sw;
        const arrowLen = Math.max(5, spaceWidth * 0.7);
        const arrowH = Math.max(3, spaceWidth * 0.42);
        const headX = endX;
        const shaftX2 = headX - arrowLen;
        return (
            `<line x1="${startX.toFixed(1)}" y1="${dy.toFixed(1)}" x2="${shaftX2.toFixed(1)}" y2="${dy.toFixed(1)}" stroke-width="${sw.toFixed(1)}" stroke-linecap="round" />` +
            `<polygon points="${headX.toFixed(1)},${dy.toFixed(1)} ${shaftX2.toFixed(1)},${(dy - arrowH).toFixed(1)} ${shaftX2.toFixed(1)},${(dy + arrowH).toFixed(1)}" />`
        );
    }
