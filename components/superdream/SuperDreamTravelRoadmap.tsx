...
export function SuperDreamTravelRoadmap() {
  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="p-6 border-indigo-500/30 bg-gradient-to-r from-slate-900/90 via-indigo-950/40 to-slate-900/90 relative overflow-hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-700 dark:text-indigo-300 text-xs font-semibold">
          ...
          {/* Mentor Profile Card */}
          <div className="flex items-center gap-3.5 bg-slate-100/90 dark:bg-slate-900/80 p-3.5 rounded-2xl border border-indigo-500/30 shadow-xs dark:shadow-lg dark:shadow-indigo-950/40 shrink-0">
            <div className="relative">
              {mentorInfo.avatar ? (
                ...
                <div className="w-12 h-12 rounded-full bg-indigo-500/20 grid place-items-center text-indigo-700 dark:text-indigo-300 ring-2 ring-indigo-400/60 font-bold text-sm">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <p className="text-xs font-bold text-slate-900 dark:text-white">{mentorInfo.name || "Faculty Mentor"}</p>
                <UserCheck className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
              </div>
              <p className="text-[11px] text-indigo-700 dark:text-indigo-300/90 line-clamp-1">{mentorInfo.title ? mentorInfo.title.split("&")[0] : "Super Dream Mentorship Board"}</p>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">{mentorInfo.officeHours || "Office Hours: By Appointment"}</p>
            </div>
          </div>
        </div>
      </GlassCard>
          ...
          return (
            <GlassCard
              ...
              className={cn(
                ...
                isSelected
                  ? "bg-amber-500/10 dark:bg-slate-900/90 border-amber-500 dark:border-amber-400/60 ring-1 ring-amber-400/40 shadow-md dark:shadow-lg dark:shadow-indigo-950/50"
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[11px] font-bold text-cyan-700 dark:text-cyan-300 font-mono bg-cyan-100/70 dark:bg-cyan-950/60 px-2 py-0.5 rounded border border-cyan-300 dark:border-cyan-800/50">
                  {m.targetLPA}
                </span>
              </div>

              <div>
                <p className="text-sm font-bold text-slate-900 dark:text-white line-clamp-1">{m.title.split(": ")[1] || m.title}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 line-clamp-2">{m.subtitle}</p>
              </div>

              <div className="pt-2.5 border-t border-slate-200 dark:border-white/10 flex items-center justify-between text-xs">
                <span className="text-slate-500 dark:text-slate-400 font-mono">
                  {m.completedTasksCount} / {m.requiredTasksCount} Tasks Done
                </span>
                {isDone ? (
                  <span className="text-emerald-600 dark:text-emerald-400 flex items-center gap-1 font-semibold text-xs">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Done
                  </span>
                ) : isCurr ? (
                  <span className="text-amber-600 dark:text-amber-400 flex items-center gap-1 font-semibold text-xs">
                    <Clock className="w-3.5 h-3.5" /> Active
                  </span>
                ) : (
                  <span className="text-slate-400 dark:text-slate-500 text-xs">Upcoming</span>
                )}
              </div>
            </GlassCard>
          );
      ...
      {activeMilestone && (
        <GlassCard className="p-5 border-indigo-500/30 bg-white/80 dark:bg-slate-900/70 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-white/10">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-slate-900 dark:text-white">{activeMilestone.title}</h3>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">{activeMilestone.description}</p>
            </div>

            <div className="text-right shrink-0">
              <p className="text-xs text-slate-500 dark:text-slate-400">Milestone Progress</p>
          ...
          <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-start gap-2.5 text-xs">
            ...
            <div>
              <span className="font-bold text-amber-700 dark:text-amber-300">Mentor Directive ({mentorInfo.name || "Faculty Mentor"}):</span>{" "}
              <span className="text-slate-700 dark:text-slate-200 italic">"{activeMilestone.mentorNotes}"</span>
            </div>
          </div>
        </GlassCard>
      )}
      ...
      <div className="space-y-3">
        ...
        {currentPhaseTasks.length === 0 ? (
          <div className="p-8 rounded-2xl bg-card/60 dark:bg-slate-900/40 border border-border/80 dark:border-white/10 text-center text-muted-foreground shadow-xs">
            <Compass className="w-8 h-8 mx-auto mb-2 opacity-60 text-indigo-500 dark:text-indigo-400" />
            <p className="text-sm font-medium text-foreground">No tasks assigned for this phase yet.</p>
            <p className="text-xs text-muted-foreground mt-1">Deliverables for this milestone will be published shortly.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentPhaseTasks.map((task) => {
              ...
              return (
                <GlassCard
                  key={task.id}
                  className="p-5 border-border/80 dark:border-slate-800 bg-card/80 dark:bg-slate-900/70 flex flex-col justify-between gap-4 card-hover-lift"
                >
                  <div>
                    ...
                    <h4 className="font-bold text-sm text-slate-900 dark:text-white leading-snug">{task.title}</h4>
                  ...
                  <div className="space-y-2.5 pt-3 border-t border-slate-200 dark:border-white/10">
                    <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                      <span>Due: <strong className="text-slate-800 dark:text-slate-200 font-mono">{task.dueDate}</strong></span>
                      <span>Priority: <strong className={task.priority === "Urgent" ? "text-rose-600 dark:text-rose-400 font-bold" : "text-amber-600 dark:text-amber-300"}>{task.priority}</strong></span>
                    </div>

                    {task.mentorFeedback && (
                      <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-indigo-500/20 text-xs">
                        <p className="text-[11px] font-semibold text-amber-700 dark:text-amber-300 flex items-center gap-1">
                          <Award className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" /> Mentor Evaluation:
                        </p>
                        <p className="text-slate-700 dark:text-slate-200 text-[11px] mt-0.5 italic">{task.mentorFeedback}</p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1">
                      {task.deliverableLink ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 flex items-center gap-1 truncate max-w-[180px]"
                        >
                          <ExternalLink className="w-3 h-3" /> View Submitted Code
                        </a>
                      ) : (
                        <span className="text-[11px] text-slate-400 dark:text-slate-500">No link attached</span>
                      )}
      ...
      {activeTaskModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/75 backdrop-blur-md animate-in fade-in">
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 border border-amber-500/30 dark:border-amber-500/40 p-6 shadow-2xl space-y-4 text-slate-900 dark:text-white relative">
            <button
            ...
            <div className="space-y-1">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">{activeTaskModal.title}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Assigned by {activeTaskModal.assignedBy}</p>
            </div>
            ...
            <form onSubmit={handleConfirmSubmission} className="space-y-4 pt-1">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 font-mono"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 resize-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setActiveTaskModal(null)}
                  className="px-4 py-2 rounded-xl text-xs font-medium bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 transition"
                >
                  Cancel
                </button>
                <button
